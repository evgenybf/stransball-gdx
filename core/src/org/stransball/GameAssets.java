...
//            defaultFont.getData().setScale(5f);
    ...
    public static class ShipAssets {
        ...
        public ShipAssets(TextureAtlas atlas) {
            ...
            float dx = shipRegion.originalWidth/2.0f;
            float dy = shipRegion.originalHeight/2.0f;
            for (int i = 0; i < shipPolygon.getVertices().length; i += 2) {
            	shipPolygon.getVertices()[i] = (shipPolygon.getVertices()[i] - dx);
            }
            for (int i = 1; i < shipPolygon.getVertices().length; i+=2) {
                shipPolygon.getVertices()[i] = -(shipPolygon.getVertices()[i] - dy);
            }
